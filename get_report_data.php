...
// 2. Total Check-In Hari Ini (Gabungan Member & Non-Member)
$q_ci_m = mysqli_query($conn, "SELECT COUNT(*) as total FROM gym_checkins WHERE DATE(checked_in_at) = CURDATE()");
$r_ci_m = mysqli_fetch_assoc($q_ci_m);
$q_ci_n = mysqli_query($conn, "SELECT COUNT(*) as total FROM daily_guests WHERE DATE(created_at) = CURDATE()");
$r_ci_n = mysqli_fetch_assoc($q_ci_n);
$total_checkin_hari_ini = (int)$r_ci_m['total'] + (int)$r_ci_n['total'];

// 3. Pemasukan Hari Ini (Tamu Harian + Member Baru Hari Ini + Perpanjangan Hari Ini)
$r_income_guest = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(payment_amount) as total FROM daily_guests WHERE DATE(created_at) = CURDATE()"));
$r_income_new = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM gym_members WHERE DATE(created_at) = CURDATE()"));
$r_income_renew = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(amount) as total FROM cashier_transactions WHERE DATE(transaction_at) = CURDATE()"));

$pemasukan_hari_ini = (int)$r_income_guest['total'] + ((int)$r_income_new['total'] * 90000) + (int)$r_income_renew['total'];
...
echo json_encode(array(
    ...
    "total_checkin" => $total_checkin_hari_ini,
